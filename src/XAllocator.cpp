...
#define XAPR_ALIGN_DEFAULT(size)   XAPR_ALIGN(size, 8)
...
size_t XAllocator::FindBlockByOffset(size_t next, size_t find, int includes)
{
    size_t prev = 0;
...
size_t XAllocator::FindBlockBySize(size_t size)
{
...
void XAllocator::MoveBlock(size_t t, int f)
{   
...
bool XAllocator::CreatePool(size_t maxLength)
{
...
void *XAllocator::Malloc(size_t reqsize)
{
    size_t size;
    size_t t;
...
void *XAllocator::Calloc(size_t reqsize)
{
...
void XAllocator::Free(void *entity)
{   
	if (entity == NULL)
	{
		return;
